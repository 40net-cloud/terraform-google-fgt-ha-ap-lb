...
run "basic-3-nic" {
  ...
  assert {
    condition     = length(google_compute_instance.fgt_vm[0].network_interface) == 3
    error_message = "Number of NICs is not equal to 3"
  }

  assert {
    condition     = local.ha_port == "port3"
    error_message = "HA port is not set to port3 (default in 3-nic deployment)"
  }

  assert {
    condition     = local.mgmt_port == "port3"
    error_message = "Management port is not set to port3 (default in 3-nic deployment)"
  }
}
...
run "licensing-flex" {
  ...
  assert {
    condition     = google_compute_instance.fgt_vm[0].network_interface[0].nic_type == "GVNIC"
    error_message = "nic0 type is not set to GVNIC (default)"
  }
}
